...
using Dating_App_Backend.Migrations;
...
namespace Dating_App_Backend.Controllers
{
    ...
    public class ChatGroupController: BaseApiController
    {
        ...
        public ChatGroupController(DataContext context, IPhotoService photoService, IMapper mapper, IHubContext<PresenceHub> presenceHub, IHubContext<MessageHub> messageHub)
        {
            _context = context;
        ...
        public async Task<ActionResult> CreateChatGroup([FromForm] CreateChatGroupDto groupDto)
        {
            ...
            var photo = new Entities.AppFile();
            ...
            var chatGroup = new Entities.ChatGroup
            {
            ...
            var participant = new Entities.ChatGroupParticipant
            {
        ...
        public async Task<ActionResult> InviteParticipants(InviteParticipantsDto inviteDto)
        {
            ...
            //notify user
            var presenceConnections = await PresenceTracker.GetUserConnections(inviteDto.ToInviteUserName);
            if (presenceConnections != null)
                await _presenceHub.Clients.Clients(presenceConnections).SendAsync("ReciveGroupInvite");
        ...
        public async Task<ActionResult> TakeActionWithInvite(InviteResponseDto inviteResponse)
        {
            ...
            if (inviteResponse.Response)
            {
                ...
                ///add system message that the user  inned the group
                var message = new Message
                {
                    ChatGroupId = invite.ChatGroupId,
                ...
                _context.Messages.Add(message);
                await _context.SaveChangesAsync();
                await _messageHub.Clients.Group(invite.ChatGroupId).SendAsync("NewMessage", _mapper.Map<MessageDto>(message));
            }
            else
            {
                //delete the invite
        ...
        public async Task<ActionResult> RemoveParticipant(int participantId, string groupId)
        {
            ...
            var message = new Message
            {
                ChatGroupId = participant.ChatGroupId,
                Content = $"{participant.ParticipantUserName} kicked out by {User.GetUsername()}",
                IsGroupMessage = true,
                IsSystemMessage = true,
                SenderUsername = "System",
                RecipenetUsername = participant.ChatGroup.GroupName,
                GroupName = participant.ChatGroupId,
            };

            _context.Messages.Add(message);
            await _context.SaveChangesAsync();

            await _messageHub.Clients.Group(participant.ChatGroupId).SendAsync("NewMessage", _mapper.Map<MessageDto>(message));
            var participantConnections = await PresenceTracker.GetUserConnections(participant.ParticipantUserName);
            if (participantConnections != null)
                await _presenceHub.Clients.Clients(participantConnections).SendAsync("YouKickedOut");
        ...
        public async Task<ActionResult> GetGroupInfo(string groupId)
        {
            ...
            var group = await _context.ChatGroups
                ...
                .Include(g => g.GroupParticipants).ThenInclude(p => p.ParticipantRole)
                .FirstOrDefaultAsync();
        ...
        public ActionResult ExitFromGroup(string groupId)
        {
            ...
            else
            {
                ...
                {
                    ChatGroupId = chatGroup.Id,
                    Content = $"{User.GetKnownAs()} left the group.",
                    IsGroupMessage = true,
                    IsSystemMessage = true,
                    SenderUsername = "System",
                    RecipenetUsername = chatGroup.GroupName,
                ...
                _context.Messages.Add(message);
                _context.SaveChanges();
                _messageHub.Clients.Group(chatGroup.Id).SendAsync("NewMessage", _mapper.Map<MessageDto>(message));
            }
        ...
        public async Task<ActionResult> PromoteParticipant(PromoteParticipantDto promoteDto)
        {
            ...
            var message = new Message
            {
                ChatGroupId = toPromoteParticipant.ChatGroup.Id,
                Content = $"{toPromoteParticipant.ParticipantUserName} is been promoted.",
                IsGroupMessage = true,
                IsSystemMessage = true,
                SenderUsername = "System",
                RecipenetUsername = toPromoteParticipant.ChatGroup.GroupName,
                GroupName = toPromoteParticipant.ChatGroup.Id,
            };

            _context.Messages.Add(message);
            await _context.SaveChangesAsync();
            await _messageHub.Clients.Group(toPromoteParticipant.ChatGroup.Id)
                .SendAsync("NewMessage", _mapper.Map<MessageDto>(message));

            return Ok();
        }

        [HttpPost("demote-participant")]
        public async Task<ActionResult> DemoteParticipant(DemoteParticipantDto demoteDto)
        {
            // Check that the caller is the group creator
            var caller = await _context.ChatGroupParticipants
                .Where(p => p.ChatGroupId == demoteDto.GroupId && p.ParticipantId == User.GetUserId())
                .Include(p => p.ParticipantRole)
                .FirstOrDefaultAsync();

            if (caller.ParticipantRole.RoleName != ParticipantRolesSrc.GroupCreator)
            {
                return Forbid();
            }

            var toDemoteParticipant = await _context.ChatGroupParticipants
                .Where(p => p.ChatGroupId == demoteDto.GroupId && p.ParticipantId == demoteDto.ParticipantId)
                .Include(p => p.ParticipantRole)
                .Include(p => p.ChatGroup)
                .FirstOrDefaultAsync();

            if (toDemoteParticipant == null)
            {
                return NotFound("Participant not in the group");
            }

            if (toDemoteParticipant.ParticipantRole.RoleName != ParticipantRolesSrc.GroupAdmin)
            {
                return BadRequest("Only group admins can be demoted");
            }

            var participantRole = await _context.ParticipantRoles.Where(r => r.RoleName == ParticipantRolesSrc.GroupParticipant)
                .FirstOrDefaultAsync();

            toDemoteParticipant.ParticipantRoleId = participantRole.Id;

            var message = new Message
            {
                ChatGroupId = toDemoteParticipant.ChatGroup.Id,
                Content = $"{toDemoteParticipant.ParticipantUserName} has been demoted.",
                IsGroupMessage = true,
                IsSystemMessage = true,
                SenderUsername = "System",
                RecipenetUsername = toDemoteParticipant.ChatGroup.GroupName,
                GroupName = toDemoteParticipant.ChatGroup.Id,
            };

            _context.Messages.Add(message);
            await _context.SaveChangesAsync();
            await _messageHub.Clients.Group(toDemoteParticipant.ChatGroup.Id)
                .SendAsync("NewMessage", _mapper.Map<MessageDto>(message));
        ...
        public async Task<ActionResult> UpdateGroupPhoto(string groupId,[Required] IFormFile newPhoto)
        {
            ...
            var photo = new Entities.AppFile(newPhoto);
